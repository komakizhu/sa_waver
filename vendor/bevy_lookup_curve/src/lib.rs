...
const MIN_TANGENT_WEIGHT: f32 = 0.02;
const MAX_TANGENT_SLOPE: f32 = 1.0e4;
...
impl Knot {
    ...
    pub fn with_tangent_slope(&self, side: TangentSide, slope: f32) -> Self {
        let mut knot = *self;
        let slope = sanitize_slope(slope);
    ...
    pub fn with_tangent_weight(&self, side: TangentSide, weight: Option<f32>) -> Self {
        let mut knot = *self;
        let weight = weight.map(sanitize_weight);
        match side {
    ...
    pub fn compute_bezier_to(&self, knot_b: &Knot) -> [Vec2; 4] {
        let slope_a = sanitize_slope(self.right_tangent.slope);
        let weight_a = sanitize_weight(self.right_tangent.weight.unwrap_or(1. / 3.));
        let slope_b = sanitize_slope(knot_b.left_tangent.slope);
        let weight_b = sanitize_weight(knot_b.left_tangent.weight.unwrap_or(1. / 3.));
...
impl LookupCurve {
    ...
    #[cfg(feature = "ron")]
    pub fn load_from_bytes(bytes: &[u8]) -> Result<Self, LookupCurveLoadError> {
        let lookup_curve = ron::de::from_bytes::<LookupCurve>(bytes)?;
        Ok(lookup_curve)
    }
...
fn unweighted_cubic_interp(knot_a: &Knot, knot_b: &Knot, x: f32) -> f32 {
    ...
    sanitize_curve_value(a * knot_a.position.y + b * m0 + c * m1 + d * knot_b.position.y)
}
...
fn weighted_cubic_interp(
    ...
    for _ in 0..max_iters {
        ...
        if !pos_guess.is_finite() {
            return fallback_segment_value(knot_a, knot_b, x);
        }
    ...
    if pos_guess.y.is_finite() {
        pos_guess.y
    } else {
        fallback_segment_value(knot_a, knot_b, x)
    }
}
...
fn sanitize_weight(weight: f32) -> f32 {
    if !weight.is_finite() {
        ...
        weight.clamp(MIN_TANGENT_WEIGHT, 1.0)
    }
}

#[inline]
fn sanitize_slope(slope: f32) -> f32 {
    if slope.is_finite() {
        slope.clamp(-MAX_TANGENT_SLOPE, MAX_TANGENT_SLOPE)
    } else if slope.is_sign_negative() {
        -MAX_TANGENT_SLOPE
    } else {
        MAX_TANGENT_SLOPE
    }
...
#[inline]
fn sanitize_curve_value(value: f32) -> f32 {
    if value.is_finite() { value } else { 0.0 }
}

#[inline]
fn fallback_segment_value(knot_a: &Knot, knot_b: &Knot, x: f32) -> f32 {
    let dx = knot_b.position.x - knot_a.position.x;
    if !dx.is_finite() || dx.abs() <= f32::EPSILON {
        return sanitize_curve_value(knot_a.position.y);
    }

    let t = ((x - knot_a.position.x) / dx).clamp(0.0, 1.0);
    sanitize_curve_value(knot_a.position.y + (knot_b.position.y - knot_a.position.y) * t)
}
